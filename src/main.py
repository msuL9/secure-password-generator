...
def main():
    ...
    parser = argparse.ArgumentParser(description="SecurePassGen: Generate or analyze strong passwords")
    ...
    mode_group.add_argument("--length", type=int, default=None, help="Password length (minimum 8 for NIST compliance)")
    ...
    if args.analyze:
        ...
        if args.length is None:
            args.length = 12
        if args.length < 8:
            print("Error: Password length must be at least 8 characters per NIST guidelines.")
            return
        try:
